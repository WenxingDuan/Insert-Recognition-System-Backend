...
def process_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Train new netword on a dataset and save the model')
...
def main():
    ...
    # Load data
    train_data, test_data, valid_data, trainloader, testloader, validloader = du.loading_data(
        input_arguments.data_directory)
    ...
    model = mu.set_model_classifier(model,
    ...
    # Train the model
    model, epochs, optimizer = mu.train_model(
        model, trainloader, input_arguments.epochs, validloader,
        input_arguments.learning_rate, default_device, choosen_architecture)
    ...
    checkpoint_file_path = os.path.join(
        input_arguments.save_directory, choosen_architecture + "_102improve" +
        str(input_arguments.epochs) + ".pth")
